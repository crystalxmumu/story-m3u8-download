...
import top.todev.story.download.m3u8.utils.Log;
import top.todev.story.download.m3u8.utils.MediaFormat;
import top.todev.story.download.m3u8.utils.StringUtils;
...
import static top.todev.story.download.m3u8.utils.Constant.FILESEPARATOR;
...
public class M3u8DownloadFactory {
    ...
    public static class M3u8Download {
        ...
        private void mergeTs() {
            try {
                File file = new File(dir + FILESEPARATOR + fileName + ".mp3");
                System.gc();
                if (file.exists())
                    file.delete();
